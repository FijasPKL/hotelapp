                        ...
                        children: [
                          ...
                          InkWell(
                            onTap: () async {
                              ...
                              /// 1️⃣ CLONE DATA
                              final Map<String, Set<String>> localSeats = selectedSeats.map(
                                    (key, value) => MapEntry(key, Set<String>.from(value)),
                              );
                              final localItems = List.from(kotProvider.selectedItemsOld);
                              final localNote = noteController.text;

                              /// 2️⃣ CLEAR UI IMMEDIATELY AND RESET BUTTON
                              setState(() {
                                selectedSeats.clear();
                                selectedItemsList.clear();
                                noteController.clear();
                                isPrintingUI = false; // RELEASE BUTTON COLOR IMMEDIATELY
                              });

                              kotProvider.clearAll();

                              /// 3️⃣ LET FLUTTER REBUILD UI
                              await Future.delayed(Duration.zero);

                              /// 4️⃣ DO HEAVY WORK IN BACKGROUND (NO BUTTON BLOCK)
                              _processKotAndPrint(kotProvider, localSeats, List<SelectedItems>.from(selectedItemsList), localNote);
                            },
                          ...
                          GestureDetector(
                            onTap: () async {
                              ...
                              List<SelectedItems>.from(
                                  kotProvider.selectedItemsListee);
